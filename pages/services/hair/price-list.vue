        ...
        <div class="max-w-screen-xl w-full px-4 mx-auto">
          ...
          <ul class="grid grid-cols-1 gap-x-6 gap-y-12 md:grid-cols-3 my-6">
            <li v-for="service in categoryData.services" :key="service.name">
              ...
              <ul>
                <li
                  v-for="(price, priceType) in service.prices"
                  :key="priceType"
                >
                  <p>{{ priceType }}: {{ price }}</p>
                </li>
...
<script setup>
const { data } = await useAsyncData("services/hair/price-list", () =>
  queryContent("services/hair/price-list").findOne()
);

console.log(data.value);

onMounted(() => {
  const primaryBg = "#9333EA";
  const primarySecondary = "#581C87";
